...
class Blobs:
    ...
    def plot_blobs(self, blob_shp=None, variable=None, k=None, mapType=None, **kwargs):
        ...
        if mapType == 'unique_values':
            color_map = kwargs.get('cmap', 'Paired')
            maps.plot_choropleth(map_shp, data, type=mapType, cmap = color_map,
            title='Blobs from Census ' + self.level + 's\nby ' + variable +
                ' (' + str(self.r.p)+' blobs)', k=k, figsize=(12,16))
        else:
            color_map = kwargs.get('cmap', 'hot_r')
            maps.plot_choropleth(map_shp, data, type=mapType,
            cmap = color_map, title='Blobs from Census ' + self.level + 's\nby ' + variable + 
                ' (' + str(self.r.p)+' blobs)', k=k, figsize=(12,16))
        print('\r             \n')
    ...
    def plot(self, variables=[]):
        ...
        if len(vars_to_plot) == 2:
            ax.scatter(self.x[:, self.cluster_vars.index(vars_to_plot[0])], 
                self.x[:, self.cluster_vars.index(vars_to_plot[1])], s=30, 
